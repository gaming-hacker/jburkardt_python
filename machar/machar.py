...
#*****************************************************************************80
#
## r4_machar() determines real machine constants.
#
#  Discussion:
#
...
#  Output:
#
#    integer IBETA, the radix for the floating-point representation.
#
#    integer IT, the number of base IBETA digits in the floating-point
#    significand.
#
#    integer IRND:
...
#      partial underflow.
#
#    integer NGRD, the number of guard digits for multiplication with
...
#
#    integer MACHEP, the largest negative integer such that
#      1.0 < 1.0 + real ( IBETA )^MACHEP,
#    except that MACHEP is bounded below by - ( IT + 3 ).
#
#    integer NEGEPS, the largest negative integer such that
#      1.0 - real ( IBETA )^NEGEPS < 1.0,
#    except that NEGEPS is bounded below by - ( IT + 3 ).
#
#    integer IEXP, the number of bits (decimal places if IBETA = 10)
#    reserved for the representation of the exponent (including the bias or
#    sign) of a floating-point number.
#
#    integer MINEXP, the largest in magnitude negative integer such
#    that
#      real ( IBETA )^MINEXP
#    is positive and normalized.
#
#    integer MAXEXP, the smallest positive power of BETA that overflows.
#
#    real EPS, the smallest positive floating-point
#    number such that
#      1.0 + EPS != 1.0.
...
#
#    real EPSNEG, a small positive floating-point 
#    number such that
#      1.0 - EPSNEG < 1.0.
...
#
#    real XMIN, the smallest non-vanishing normalized
#    floating-point power of the radix:
#      XMIN = real ( IBETA )^MINEXP
#
#    real XMAX, the largest finite floating-point 
...
#
  ...
  print ( '  r4_machar() computes R4 machine constants.' )
...
#    integer MAXEXP, the smallest positive power of BETA that overflows.
#
#    real EPS, the smallest positive floating-point number
#    such that
#      1.0 + EPS != 1.0.
#    in particular, if either IBETA = 2  or IRND = 0,
#      EPS = real ( IBETA )^MACHEP.
...
#      EPS = ( real ( IBETA )^MACHEP ) / 2.
#
#    real EPSNEG, a small positive floating-point number
#    such that
#      1.0 - EPSNEG < 1.0.
#    In particular, if IBETA = 2 or IRND = 0,
#      EPSNEG = real ( IBETA )^NEGEPS.
...
#      XMIN = real ( IBETA )^MINEXP
#
#    real XMAX, the largest finite floating-point number.
#    In particular,
#      XMAX = ( 1.0 - EPSNEG ) * real ( IBETA )^MAXEXP
#    On some machines, the computed value of XMAX will be only the second,
#    or perhaps third, largest number, being too small by 1 or 2 units in
...
#
  ...
  print ( '  r8_machar() computes R8 machine constants.' )
