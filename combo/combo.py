...
def npart_rsf_lex_random ( n, npart ):
...
#
  ...
  rank = np.random.random_integers ( 1, npartitions )
...
#
  ...
  for i in range ( 0, 10 ):
    ...
    t = npart_rsf_lex_random ( n, npart )
...
#
  ...
  for j in range ( 0, n ):
    ...
    rank = rank + ( p2[j] - 1 ) * np.math.factorial ( n - j )
...
#
    r1 = ( rank * np.math.factorial ( j ) ) // np.math.factorial ( n )
    k = r1 - j * r2
...
#
    rank = np.random.random_integers ( i4_lo, i4_hi )
...
#
  ...
  s = np.random.random_integers ( 0, 1, size = n )
