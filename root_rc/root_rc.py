...
#  Input:
#
#    real X, an estimate for the root.  On the first
...
#
#    real Q(9), storage needed by the function.
#    Before the first call, the user must set q[0] to 0.
#
#  Output:
#
#    real XNEW, an improved estimate for the root.
#
#    real FERR, the smallest value of F encountered.
#
#    real XERR, the width of the change-in-sign interval,
...
#
  ...
  ferr = abs ( fx )
#
#  If this is the first time, initialize, estimate the first root, and return.
#
  if ( q[0] == 0.0 ):
    q[0] = fx
    q[1] = x
...
#
  if ( 80.0 < q[8] ):
    ...
    raise Exception ( 'root_rc - Fatal error!' )
#
